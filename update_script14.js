...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function addOptionHeadings(selects) {
        ...
        // Add border bottom to the last option by inserting a full-width row
        if (selects.length > 0) {
            ...
            if (lastTr && !lastTr.dataset.borderAdded) {
                ...
                // Clear any inline border we might have added previously on td/th
                var tds = lastTr.querySelectorAll('th, td');
                tds.forEach(function(td) {
                    td.style.borderBottom = 'none';
                    td.style.paddingBottom = '15px'; // keep padding inside the select row
                });
                
                // Insert a dummy row below it to draw the full-width border
                var borderTr = document.createElement('tr');
                borderTr.innerHTML = '<td colspan="2" style="width:100%; padding:0; border-top:1px solid #e5e5e5;"></td>';
    ...
    function cleanOptions() {
        ...
        var node;
        while ((node = walker.nextNode())) {
            // Remove "(최소주문수량 1개 이상)" and similar text anywhere it appears
            if (node.nodeValue.includes('최소주문수량')) {
                node.nodeValue = node.nodeValue.replace(/\\(최소주문수량\\s*[0-9]+개\\s*이상\\)/g, '').trim();
            }
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Fixed border width in ' + f);
    }
});
