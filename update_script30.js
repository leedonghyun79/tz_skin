...
<style>
    ...
    .xans-product-action > div:not(.soldout) > button#actionCart,
    .xans-product-action > div:not(.soldout) > button#actionCartClone {
        width: 49% !important;
    ...
    /* Ensure Cart button inside the PC span takes up full width of its half */
    ...
    /* Center text inside anchor tags */
    .xans-product-action a {
    ...
    function addOptionHeadings(selects) {
        ...
        if (selects.length > 0) {
            ...
            if (lastTr && !lastTr.dataset.borderAdded) {
                ...
                // CHANGED: Made the final border thick/dark (#333) as requested by the user
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Fixed final border thickness in ' + f);
    }
});
