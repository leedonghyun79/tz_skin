...
<style>
    ...
    /* ----------------------------------------------------
       Bulletproof Option Table Alignment
       We add .custom-option-table to any table containing an option via JS.
       This bypasses all of Cafe24's dynamic wrappers and ensures 100% width.
       ---------------------------------------------------- */
    ...
    /* Ensure all selects inside option tables take full width */
    table.custom-option-table select {
        width: 100% !important;
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Applied bulletproof .custom-option-table logic in ' + f);
    }
});
