...
<style>
    ...
    /* ----------------------------------------------------
       ...
       CRITICAL: Use .xans-product-action instead of [module=...] 
       because Cafe24 strips the module attribute in the browser.
       ---------------------------------------------------- */
       
    /* Parent Clearfix */
    .xans-product-action > div:not(.soldout) {
        overflow: hidden !important;
        display: block !important; /* Overrides flex if it was there */
        width: 100% !important;
        box-sizing: border-box !important;
    }
    
    /* Set common child styles and strict 50px height */
    ...
    .xans-product-action .ec-base-button > button#actionCartClone {
        ...
        /* Flex to perfectly center text */
        display: flex !important;
        align-items: center !important;
        justify-content: center !important;
    }
    
    /* Buy Now Button (Anchor Tag) - Right Side */
    .xans-product-action > div:not(.soldout) > a,
    .xans-product-action .ec-base-button > a {
    ...
    /* Cart Button Container (span in PC, button in Mobile) - Left Side */
    .xans-product-action > div:not(.soldout) > span.gActionButtonColumn,
    .xans-product-action .ec-base-button > button#actionCart,
    .xans-product-action .ec-base-button > button#actionCartClone {
    ...
    /* Explicit Cart Button Styling (White background, Black border) */
    ...
    /* Disable Hover Effects for Both Buttons */
    ...
    /* Ensure Cart button inside the PC span takes up full width/height of its half */
    span.gActionButtonColumn > #actionCart,
    span.gActionButtonColumn > .actionCart {
        display: flex !important;
        align-items: center !important;
    ...
    function addOptionHeadings(selects) {
        ...
        if (selects.length > 0) {
            ...
            if (lastTr && !lastTr.dataset.borderAdded) {
                ...
                borderTr.innerHTML = '<td class="custom-heading-td" colspan="2" style="width:100%; padding:0 !important; border-top:1px solid #333;"></td>';
    ...
    function adjustRowPaddings() {
        ...
        trs.forEach(function(tr) {
            ...
            if (th && td) {
                ...
                if (title === '상품간략설명') {
                    // Keep original top padding, but REDUCE bottom padding to make it close to next item
                    ...
                    // REDUCE top padding to make it close to previous item, but keep original bottom padding
                    th.style.setProperty('padding-top', '5px', 'important');
                    td.style.setProperty('padding-top', '5px', 'important');
                } else if (title !== '') {
                    // Reduce BOTH top and bottom padding for intermediate rows
                    th.style.setProperty('padding-top', '5px', 'important');
                    th.style.setProperty('padding-bottom', '5px', 'important');
                    td.style.setProperty('padding-top', '5px', 'important');
                    td.style.setProperty('padding-bottom', '5px', 'important');
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Fixed row padding logic to ensure equal visual spacing internally in ' + f);
    }
});
