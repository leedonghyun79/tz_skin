...
<style>
    ...
    /* Force tables to be 100% */
    .infoArea table, .ec-base-product table {
        width: 100% !important;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function syncThWidths() {
        var optionTables = document.querySelectorAll('.infoArea table, .ec-base-product table, table[summary="상품 옵션"]');
        var maxThWidth = 0;
        var targetThs = [];
        
        optionTables.forEach(function(table) {
            var ths = table.querySelectorAll('th');
            ths.forEach(function(th) {
                if (th.closest('.custom-option-heading')) return;
                var td = th.nextElementSibling;
                if (td && (td.querySelector('select') || td.querySelector('input[type="text"]') || td.querySelector('.ec-base-qty'))) {
                    targetThs.push(th);
                    // Force nowrap to measure natural text width
                    th.style.width = 'auto';
                    th.style.whiteSpace = 'nowrap';
                    var w = th.getBoundingClientRect().width;
                    if (w > maxThWidth) maxThWidth = w;
                }
            });
        });
        
        if (maxThWidth > 0) {
            maxThWidth += 20; // Add 20px buffer for safety
            if (maxThWidth < 120) maxThWidth = 120; // set a reasonable minimum
            
            targetThs.forEach(function(th) {
                th.style.width = maxThWidth + 'px';
                th.style.minWidth = maxThWidth + 'px';
                th.style.maxWidth = maxThWidth + 'px';
                th.style.wordBreak = 'keep-all';
                th.style.whiteSpace = 'normal'; // restore wrapping if forced
            });
        }
    }
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Fixed TH alignment dynamically in ' + f);
    }
});
