...
<style>
    ...
    .infoArea select, .ec-base-product select {
        width: 100% !important;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function addOptionHeadings(selects) {
        selects.forEach(function(select, index) {
            ...
            if (tr && !tr.dataset.headingAdded) {
                ...
                if (titleText !== "") {
                    ...
                    // Make the border darker as requested (e.g. #333)
                    var borderTop = isFirst ? 'none' : '1px solid #333';
        ...
        if (selects.length > 0) {
            ...
            if (lastTr && !lastTr.dataset.borderAdded) {
                ...
                // Make the bottom border darker too to match
                borderTr.innerHTML = '<td colspan="2" style="width:100%; padding:0; border-top:1px solid #333;"></td>';
    ...
    function cleanOptions() {
        // Target ALL selects within the product options area
        ...
        // Filter out hidden ones or those not related to options (if any)
        ...
        var node;
        while ((node = walker.nextNode())) {
            // Aggressive removal of minimum quantity text
            if (node.nodeValue.includes('최소주문수량') || node.nodeValue.includes('최소 주문 수량')) {
                node.nodeValue = node.nodeValue.replace(/\\(?\\s*최소\\s*주문\\s*수량[^)]*\\)?/g, '').trim();
                node.nodeValue = node.nodeValue.replace(/\\(?\\s*최소주문수량[^)]*\\)?/g, '').trim();
            }
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Fixed select width, darker border, removed min quantity text in ' + f);
    }
});
