...
<style>
    ...
    .xans-product-detail .ec-base-button:not(.soldout) {
    ...
    .xans-product-detail .ec-base-button:not(.soldout) > a.btnSubmit, 
    .xans-product-detail .ec-base-button:not(.soldout) > span.gActionButtonColumn,
    .xans-product-detail .ec-base-button:not(.soldout) > button.btnSubmit {
        flex: 1 !important;
        width: 50% !important;
        margin: 0 !important;
        box-sizing: border-box !important;
    ...
    /* Ensure Cart takes up full width of its half */
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Made cart and buy buttons 50/50 flex side-by-side in ' + f);
    }
});
