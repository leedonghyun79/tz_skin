    ...
    function adjustRowPaddings() {
        ...
        trs.forEach(function(tr) {
            ...
            if (th && td) {
                ...
                // Check if it's the rows we want to keep normal
                if (title === '상품간략설명' || title === '배송비' || title === '할인판매가') {
                    // Leave padding normal
                } else if (title !== '') {
                    // Reduce padding for intermediate rows
...
files.forEach(f => {
    if(fs.existsSync(f)) {
        ...
        console.log('Added dynamic row padding logic in ' + f);
    }
});
